...
- **Verified end-to-end (2026-07-12):** with pipeline diagnostics on both sides, a real walk test produced the full healthy timeline — CameraX cold-start event, in-stream tap events every ~5s while motion continued, window expiry 10s after motion stopped, 45s clip. A torch-pulse repro (remotely toggling the flashlight as a controlled motion source) independently produced a 52s window. Diagnostics left in place: the hub logs every `EVENT:MOTION` with `rec/active/streaming` state and window expiries; the cam logs tap-frame delivery and each event sent.
